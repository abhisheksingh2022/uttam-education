...
import com.uttam.web.framework.common.ValidationMessageFormatter;
...
public class ErrorMessageTag extends BaseCustomTag {
	...
	private static final Logger LOG = LogManager.getLogger(ErrorMessageTag.class);
	...
	public StringBuffer startHTML() {
		...
		Map fieldErrors = (Map) ActionContext.getContext().getValueStack().findValue("fieldErrors");
		Collection actionErrors = (Collection) ActionContext.getContext().getValueStack().findValue("actionErrors");
		Collection actionMessages = (Collection) ActionContext.getContext().getValueStack().findValue("actionMessages");
		Collection<UserMessage> businessMessages = (Collection<UserMessage>) ActionContext.getContext().getValueStack()
				.findValue("businessMessages");
		Collection<String> unclassifiedMessages = (Collection<String>) pageContext.getRequest().getAttribute(
				"randomMessages");
		LOG.debug("Checking presence of any error message");
		if ((fieldErrors != null && fieldErrors.size() > 0) || (actionErrors != null && actionErrors.size() > 0)
				|| (actionMessages != null && actionMessages.size() > 0)
				|| (businessMessages != null && businessMessages.size() > 0) || unclassifiedMessages != null
		...
		if (!hideTable) {
			...
			content.append("<SPAN ID=\"").append(getPortletNameSpace()).append("errorSpanClient\">");
		}
		if (fieldErrors != null && fieldErrors.size() > 0) {
			LOG.debug("Field Error Messages present. Adding..");
			for (Iterator<String> iter = fieldErrors.keySet().iterator(); iter.hasNext();) {
				content.append(addStringErrors(((Collection) fieldErrors.get(iter.next()))));
			}
		}
	...
	private String showHelpButtonMessage() {
		...
		message.append("<IMG SRC=\"").append(getImage(ValidationMessageFormatter.getImage(3)));
		message.append("\" ALT=\"");
	...
	private String addBusinessErrors(Collection<UserMessage> businessMessages) {
		...
		if (businessMessages != null) {
			for (UserMessage aMsg : businessMessages) {
				message.append("<IMG SRC=\"").append(getImage(ValidationMessageFormatter.getImage(aMsg.getSeverity())));
				message.append("\" ALT=\"");
				message.append(getI18NText(ValidationMessageFormatter.getImageAlt(aMsg.getSeverity())));
				message.append("\" ");
				message.append("CLASS=\"imgPrp1\">&#160;");
				...
				if (aMsg.getSeverity() == 1 || aMsg.getSeverity() == 2) {
					...
					message.append("#008b00");
				}
				message.append(";\">");
				if (aMsg.getErrorIdNbr() != null) {
					message.append("Ref #: <b>ERR000").append(aMsg.getErrorIdNbr()).append("</b>: ");
				}
	...
	private String addStringErrors(Collection errors) {
		...
		if (errors != null) {
			for (Iterator iter = errors.iterator(); iter.hasNext();) {
				message.append("<IMG SRC=\"").append(getImage(ValidationMessageFormatter.getImage(1)));
				message.append("\" ALT=\"");
				message.append(getI18NText(ValidationMessageFormatter.getImageAlt(1)));
				message.append("\" ");
				message.append("CLASS=\"imgPrp1\">&#160;");
				message.append("<SPAN STYLE=\"color:red;\">");
				message.append(iter.next().toString());
				message.append("</SPAN><br/>");
	...
	private String startTable() {
		StringBuilder content = new StringBuilder();
		if (!leftMenuEnabled) {
			content.append("\n \t \t \t<TABLE ");//commented width - 03142011 - admin
		} else {
			content.append("\n \t \t \t<TABLE ");//commented width - 03142011 - admin
		}
		content.append("ID=\"").append(getPortletNameSpace()).append("errorTableClient\" class=\"errorTableClient\" ");
		...
		content.append("<SPAN ID=\"").append(getPortletNameSpace()).append("errorHeader\"><b>");
		content.append(getHeader());
		content.append("</b></SPAN>");
		content.append("</TD>");
		content.append("\n \t \t \t \t</TR>");
