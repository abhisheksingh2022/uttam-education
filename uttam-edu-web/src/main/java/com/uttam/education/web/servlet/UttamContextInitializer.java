...
public class UttamContextInitializer implements ServletContextListener {

	private static Logger LOG = null;
	static {
		try {
			initializeLog4J();
			LOG = LogManager.getLogger(UttamContextInitializer.class);
			LOG.entry();
		} catch (Exception ex) {
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		try {
			...
			try {
				...
				LogManager.getLogger(UttamContextInitializer.class).error(
